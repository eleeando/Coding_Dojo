...
class Recipe:
    ...
    def save_recipe(cls, data): #!CREATE
        query="INSERT INTO recipes (user_id,name,description,instructions,date,under) VALUES (%(user_id)s,%(name)s,%(description)s,%(instructions)s,%(date)s,%(under)s);"
    ...
    def update_recipe(cls,data): #!UPDATE//EDIT
        query="""UPDATE recipes SET name=%(name)s,
                description=%(description)s,instructions=%(instructions)s,date=%(date)s,under=%(under)s 
                WHERE id=%(id)s;"""
        return connectToMySQL(DATABASE).query_db(query,data)
